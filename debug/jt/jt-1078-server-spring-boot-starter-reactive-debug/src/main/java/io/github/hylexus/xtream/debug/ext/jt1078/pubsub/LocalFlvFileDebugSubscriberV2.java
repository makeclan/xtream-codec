...
// @Component
public class LocalFlvFileDebugSubscriberV2 implements DisposableBean {
    private static final Logger log = LoggerFactory.getLogger(LocalFlvFileDebugSubscriberV2.class);
    private final Jt1078RequestPublisher publisher;
    private final Map<String, OutputStream> registry = new HashMap<>();
    private final String targetDir;

    public LocalFlvFileDebugSubscriberV2(Jt1078RequestPublisher publisher) {
        this.publisher = publisher;
        this.targetDir = "/tmp/jtt/";
        XtreamFiles.mkdirs(this.targetDir);
    }

    public void subscribe(String sim1, short channelNumber1) {
        new Thread(() -> {
            ...
            final OutputStream outputStream = getOutputStream(sim1, channelNumber1);
