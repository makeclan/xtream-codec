...
class JtStyleDebugEntity02FlattenTest extends BaseEntityCodecTest {
    ...
    @Test
    void test() {
        final JtStyleDebugEntity02Flatten originalEntity = this.createEntity();
        final String hexString = this.encodeAsHexString(originalEntity);
        final JtStyleDebugEntity02Flatten entity = this.doDecode(JtStyleDebugEntity02Flatten.class, hexString);
        ...
        for (int i = 0; i < originalEntity.getExtraItems().size(); i++) {
            final JtStyleDebugEntity02Flatten.ExtraItem original = originalEntity.getExtraItems().get(i);
            final JtStyleDebugEntity02Flatten.ExtraItem decoded = entity.getExtraItems().get(i);
    ...
    JtStyleDebugEntity02Flatten createEntity() {
        final JtStyleDebugEntity02Flatten entity = new JtStyleDebugEntity02Flatten();
        this.initBasicProperties(entity);
        entity.setExtraItems(List.of(
                new JtStyleDebugEntity02Flatten.ExtraItem((short) 0x01, (short) 4, ByteArrayContainer.ofDword(6666).asBytes()),
                new JtStyleDebugEntity02Flatten.ExtraItem((short) 0x02, (short) 2, ByteArrayContainer.ofI16(88).asBytes()),
                new JtStyleDebugEntity02Flatten.ExtraItem((short) 0x03, (short) 2, ByteArrayContainer.ofI16(89).asBytes())
        ));
        this.initMsgBodyProps(entity, entity.getExtraItems());
        return entity;
    }

    private void initMsgBodyProps(JtStyleDebugEntity02Flatten entity, List<JtStyleDebugEntity02Flatten.ExtraItem> extraItemList) {
    ...
    private void initBasicProperties(JtStyleDebugEntity02Flatten entity) {
