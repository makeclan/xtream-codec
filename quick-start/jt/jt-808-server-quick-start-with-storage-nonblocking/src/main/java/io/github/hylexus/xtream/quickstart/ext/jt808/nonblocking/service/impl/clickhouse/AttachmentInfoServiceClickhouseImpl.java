...
public class AttachmentInfoServiceClickhouseImpl implements AttachmentInfoService {
    ...
    public Mono<Boolean> saveAlarmInfo(String terminalId, String filePath, BuiltinMessage1210.AttachmentItem attachmentItem) {
        ...
        return DatabaseRouter.TRACE_LOG_CLICKHOUSE.executeMono(this.mapper.insert(entity))
                .then(Mono.just(true));
    }

    @Override
    public Mono<PageableVo<Jt808AlarmAttachmentInfoVo>> listAlarmAttachmentInfo(Jt808AlarmAttachmentInfoDto dto) {
        return DatabaseRouter.TRACE_LOG_CLICKHOUSE.executeMono(this.mapper.count(dto)).flatMap(total -> {
            if (total <= 0) {
                return Mono.just(PageableVo.empty());
            }
            return DatabaseRouter.TRACE_LOG_CLICKHOUSE.executeFlux(this.mapper.list(dto)).collectList().map(list -> {
