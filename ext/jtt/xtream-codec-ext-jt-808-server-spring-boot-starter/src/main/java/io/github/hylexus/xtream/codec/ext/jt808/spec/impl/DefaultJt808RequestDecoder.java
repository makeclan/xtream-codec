...
public class DefaultJt808RequestDecoder implements Jt808RequestDecoder {
    ...
    protected final Jt808MessageProcessor jt808MessageProcessor;

    public DefaultJt808RequestDecoder(Jt808MessageProcessor jt808MessageProcessor) {
    ...
    protected Jt808RequestHeader parseMsgHeaderSpec(ByteBuf byteBuf) {
        // 1. bytes[0-1] WORD
        final int msgId = XtreamBytes.getWord(byteBuf, 0);
        // bytes[2-3] WORD 消息体属性
        final int msgBodyPropsIntValue = XtreamBytes.getWord(byteBuf, 2);
        final Jt808RequestHeader.Jt808MsgBodyProps msgBodyProps = new DefaultJt808MsgBodyProps(msgBodyPropsIntValue);

        final Jt808ProtocolVersion version = this.detectVersion(msgId, msgBodyProps, byteBuf);
    ...
    protected Jt808ProtocolVersion detectVersion(int msgId, Jt808RequestHeader.Jt808MsgBodyProps msgBodyProps, ByteBuf byteBuf) {
